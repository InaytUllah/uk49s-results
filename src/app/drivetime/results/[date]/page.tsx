...
export default async function DrivetimeResultPage({ params }: Props) {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Drivetime', item: `${SITE_URL}/drivetime` },
      { '@type': 'ListItem', position: 3, name: `Results ${formattedDate}`, item: `${SITE_URL}/drivetime/results/${date}` },
    ],
  };
  const articleSchema = result ? {
    '@context': 'https://schema.org',
    '@type': 'NewsArticle',
    headline: `UK 49s Drivetime Results — ${formattedDate}`,
    description: `UK 49s Drivetime winning numbers for ${formattedDate}: ${result.numbers.join(', ')} with Booster ${result.booster}.`,
    datePublished: date,
    dateModified: date,
    author: { '@type': 'Organization', name: SITE_NAME, url: SITE_URL },
    publisher: { '@type': 'Organization', name: SITE_NAME, url: SITE_URL },
    mainEntityOfPage: { '@type': 'WebPage', '@id': `${SITE_URL}/drivetime/results/${date}` },
  } : null;
