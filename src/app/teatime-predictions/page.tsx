...
export default async function TeatimePredictionsPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {(() => {
        const pastDates = [...new Set(teatimeResults.map(r => r.date))]
          .sort((a, b) => b.localeCompare(a))
          .filter(d => d !== predInfo.date)
          .slice(0, 5);
        if (pastDates.length === 0) return null;
        return (
          <section className="mb-10">
            <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-4">Past Teatime Predictions</h2>
            ...
            <ul className="space-y-2">
              {pastDates.map(date => {
                ...
                return (
                  <li key={date}>
                    <Link
                      href={`/blog/uk-49s-teatime-predictions-${date}`}
                    ...
                    >
                      <span className="text-sm font-medium text-gray-900 dark:text-white">UK 49s Teatime Predictions for {formatted}</span>
                      <svg aria-hidden="true" className="w-4 h-4 text-indigo-600 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
                    </Link>
                  </li>
                );
              })}
