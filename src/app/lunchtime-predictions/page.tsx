...
export const metadata: Metadata = {
  title: `UK 49s Lunchtime Predictions Today — Hot Numbers & Analysis | ${SITE_NAME}`,
  description: 'Get today\'s UK 49s Lunchtime predictions based on statistical analysis. Hot & cold number trends, frequency patterns, and 3 weighted prediction sets for the 12:49 PM draw.',
  alternates: { canonical: '/lunchtime-predictions' },
  openGraph: {
    title: 'UK 49s Lunchtime Predictions Today',
    description: 'Statistical predictions for today\'s UK 49s Lunchtime draw at 12:49 PM.',
    type: 'article',
    images: [{
      url: `${SITE_URL}/api/og?title=${encodeURIComponent('Lunchtime Predictions Today')}&subtitle=${encodeURIComponent('12:49 PM Draw — Statistical Analysis')}&type=prediction`,
  ...
  twitter: {
    ...
    title: 'UK 49s Lunchtime Predictions Today',
    images: [`${SITE_URL}/api/og?title=${encodeURIComponent('Lunchtime Predictions Today')}&subtitle=${encodeURIComponent('12:49 PM Draw — Statistical Analysis')}&type=prediction`],
  },
};
...
export default async function LunchtimePredictionsPage() {
  ...
  const hot = getHotNumbers(lunchtimeResults, 10);
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      <nav aria-label="Breadcrumb" className="mb-4 text-sm text-gray-500 dark:text-gray-400">
        <ol className="flex items-center gap-1">
          ...
          <li><span className="mx-1">/</span></li>
          <li className="text-gray-900 dark:text-white font-medium">Lunchtime</li>
        </ol>
      </nav>

      <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white mb-2">
        UK 49s Lunchtime Predictions Today
      </h1>
      <p className="text-gray-600 dark:text-gray-400 mb-2">
        Statistical predictions for the {lunchInfo.formatted} draw at 12:49 PM UK time
      </p>
      ...
      <section className="mb-10">
        <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 sm:p-8">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">Lunchtime Trending Numbers</h2>
      ...
      {/* Hot & Cold */}
      <section className="mb-10">
        <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-4">Lunchtime Hot & Cold Numbers</h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Link href="/teatime-predictions" className="px-4 py-3 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 transition-colors font-medium text-center text-sm">
          Teatime Predictions
        </Link>
        <Link href="/lunchtime" className="px-4 py-3 bg-amber-500 text-white rounded-lg hover:bg-amber-600 transition-colors font-medium text-center text-sm">
          Lunchtime Results
        </Link>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        '@type': 'FAQPage',
        mainEntity: [
          { '@type': 'Question', name: 'What are the UK 49s Lunchtime predictions for today?', acceptedAnswer: { '@type': 'Answer', text: `Our statistical predictions for the ${lunchInfo.formatted} Lunchtime draw use weighted analysis of hot numbers from recent draws. We provide 3 unique prediction sets combining frequently drawn numbers with random selections.` } },
          { '@type': 'Question', name: 'How are UK 49s Lunchtime predictions calculated?', acceptedAnswer: { '@type': 'Answer', text: 'Predictions are generated by analysing number frequency patterns in recent Lunchtime draws. 4 numbers are selected from the top 10 most frequently drawn, and 2 are randomly selected for balance. Each draw is independent so Lunchtime data is used separately from Teatime.' } },
          { '@type': 'Question', name: 'When do Lunchtime predictions update?', acceptedAnswer: { '@type': 'Answer', text: 'Lunchtime predictions automatically update to the next day once the 12:49 PM draw results are announced. This ensures predictions always target the upcoming draw.' } },
        ],
      }) }} />
    </div>
  );
