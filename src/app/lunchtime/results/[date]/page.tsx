...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `UK 49s Lunchtime Results ${formattedDate} | ${SITE_NAME}`,
    description: `UK 49s Lunchtime draw results for ${formattedDate}. ${numbersText}Full analysis, hot & cold numbers, and draw breakdown.`,
    alternates: {
      canonical: `${SITE_URL}/lunchtime/results/${date}`,
    },
    openGraph: {
      title: `UK 49s Lunchtime Results — ${formattedDate}`,
      description: `${numbersText}Full draw analysis and hot number breakdown.`,
      type: 'article',
      images: [{
        url: `${SITE_URL}/api/og?title=${encodeURIComponent(`Lunchtime Results — ${formattedDate}`)}&subtitle=${encodeURIComponent('UK 49s Winning Numbers')}&type=result&numbers=${ogNumbers}&booster=${ogBooster}`,
        width: 1200,
        height: 630,
        alt: `UK 49s Lunchtime Results for ${formattedDate}`,
      }],
    },
    twitter: {
      card: 'summary_large_image',
      title: `UK 49s Lunchtime Results — ${formattedDate}`,
      images: [`${SITE_URL}/api/og?title=${encodeURIComponent(`Lunchtime Results — ${formattedDate}`)}&subtitle=${encodeURIComponent('UK 49s Winning Numbers')}&type=result&numbers=${ogNumbers}&booster=${ogBooster}`],
    },
  };
}
...
export default async function LunchtimeResultPage({ params }: Props) {
  const { date } = await params;
  const result = await getResultByDate(date, 'lunchtime');
  const allLunchtime = await getLatestResults('lunchtime');
  const hot = getHotNumbers(allLunchtime, 7);
  const cold = getColdNumbers(allLunchtime, 7);

  const formattedDate = new Date(date + 'T00:00:00').toLocaleDateString('en-GB', {
    weekday: 'long', year: 'numeric', month: 'long', day: 'numeric',
  });

  // Check how many of today's numbers are in the hot list
  ...
  const faqSchema = result ? {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `What were the UK 49s Lunchtime results on ${formattedDate}?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The UK 49s Lunchtime winning numbers on ${formattedDate} were ${result.numbers.join(', ')} with Booster ball ${result.booster}.`,
        },
      },
      {
        '@type': 'Question',
        name: `What time was the UK 49s Lunchtime draw on ${formattedDate}?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The UK 49s Lunchtime draw on ${formattedDate} took place at 12:49 PM UK time, as scheduled every day.`,
        },
      },
      {
        '@type': 'Question',
        name: `Were any hot numbers drawn in the UK 49s Lunchtime on ${formattedDate}?`,
        acceptedAnswer: {
          '@type': 'Answer',
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <nav className="text-sm text-gray-500 dark:text-gray-400 mb-6">
        ...
        <Link href="/lunchtime" className="hover:text-emerald-600">Lunchtime</Link>
        <span className="mx-2">/</span>
        <span className="text-gray-900 dark:text-white">{formattedDate}</span>
      </nav>

      <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-6">
        UK 49s Lunchtime Results — {formattedDate}
      </h1>
      ...
      {result ? (
        <>
          ...
          <section className="mb-8">
            <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 sm:p-8">
              ...
              <p className="text-gray-600 dark:text-gray-400 leading-relaxed mb-3">
                The <strong className="text-gray-900 dark:text-white">UK 49s Lunchtime</strong> draw on <strong className="text-gray-900 dark:text-white">{formattedDate}</strong> produced the winning numbers{' '}
          ...
          <section className="mb-8">
            <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 sm:p-8">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">About the UK 49s Lunchtime Draw</h2>
              <p className="text-gray-600 dark:text-gray-400 leading-relaxed">
                The <strong className="text-gray-900 dark:text-white">UK 49s Lunchtime</strong> draw takes place every day at <strong className="text-gray-900 dark:text-white">12:49 PM</strong> UK time. Six main numbers are drawn from a pool of 1 to 49, followed by a <strong className="text-gray-900 dark:text-white">Booster ball</strong>. Players can choose to bet on 1 to 5 numbers with various prize tiers. Check our <Link href="/odds" className="text-emerald-600 dark:text-emerald-400 hover:underline font-medium">odds and payouts</Link> page for full details on how betting works.
              </p>
            </div>
          </section>
        </>
