...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `UK 49s Teatime draw results for ${formattedDate}. ${numbersText}Full analysis, hot & cold numbers, and draw breakdown.`,
    alternates: {
...
export default async function TeatimeResultPage({ params }: Props) {
  const { date } = await params;
  const result = await getResultByDate(date, 'teatime');
  const allTeatime = await getLatestResults('teatime');
  const hot = getHotNumbers(allTeatime, 7);
  const cold = getColdNumbers(allTeatime, 7);
  ...
  const faqSchema = result ? {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `What were the UK 49s Teatime results on ${formattedDate}?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The UK 49s Teatime winning numbers on ${formattedDate} were ${result.numbers.join(', ')} with Booster ball ${result.booster}.`,
        },
      },
      {
        '@type': 'Question',
        name: `What time was the UK 49s Teatime draw on ${formattedDate}?`,
        acceptedAnswer: {
          '@type': 'Answer',
          text: `The UK 49s Teatime draw on ${formattedDate} took place at 5:49 PM UK time, as scheduled every day.`,
        },
      },
      {
        '@type': 'Question',
        name: `Were any hot numbers drawn in the UK 49s Teatime on ${formattedDate}?`,
        acceptedAnswer: {
          '@type': 'Answer',
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Teatime', item: `${SITE_URL}/teatime` },
      { '@type': 'ListItem', position: 3, name: `Results ${formattedDate}`, item: `${SITE_URL}/teatime/results/${date}` },
    ],
  };
  const articleSchema = result ? {
    '@context': 'https://schema.org',
    '@type': 'NewsArticle',
    headline: `UK 49s Teatime Results — ${formattedDate}`,
    description: `UK 49s Teatime winning numbers for ${formattedDate}: ${result.numbers.join(', ')} with Booster ${result.booster}.`,
    datePublished: date,
    dateModified: date,
    author: { '@type': 'Organization', name: SITE_NAME, url: SITE_URL },
    publisher: { '@type': 'Organization', name: SITE_NAME, url: SITE_URL },
    mainEntityOfPage: { '@type': 'WebPage', '@id': `${SITE_URL}/teatime/results/${date}` },
  } : null;
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {result ? (
        <>
          ...
          <section className="mb-8">
            <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 sm:p-8">
              ...
              <p className="text-gray-600 dark:text-gray-400 leading-relaxed mb-3">
                The <strong className="text-gray-900 dark:text-white">UK 49s Teatime</strong> draw on <strong className="text-gray-900 dark:text-white">{formattedDate}</strong> produced the winning numbers{' '}
          ...
          <section className="mb-8">
            <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 sm:p-8">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">About the UK 49s Teatime Draw</h2>
              <p className="text-gray-600 dark:text-gray-400 leading-relaxed">
                The <strong className="text-gray-900 dark:text-white">UK 49s Teatime</strong> draw takes place every day at <strong className="text-gray-900 dark:text-white">5:49 PM</strong> UK time. Six main numbers are drawn from a pool of 1 to 49, followed by a <strong className="text-gray-900 dark:text-white">Booster ball</strong>. Players can choose to bet on 1 to 5 numbers with various prize tiers. Check our <Link href="/odds" className="text-emerald-600 dark:text-emerald-400 hover:underline font-medium">odds and payouts</Link> page for full details on how betting works.
              </p>
            </div>
          </section>
        </>
